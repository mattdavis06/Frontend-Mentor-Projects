...
export default function Nav() {
	...
	return (
		<nav className='col-span-1 row-span-1 lg:col-span-3 flex items-center justify-between'>
			...
			<div className='nav-menu-desktop hidden lg:block'>
				<ul className='flex'>
					{navLinks.map((link) => {
						return (
							<li className='px-5' key={uuid()}>
								<a
									className='text-darkGrayishBlue text-lg hover:text-primaryOrange transition ease-in-out'
			...
			<div
				className={`nav-menu-mobile bg-veryDarkBlue/70 fixed w-full h-full top-0 transition ease-in-out duration-500 ${
					isOpen ? 'left-0' : 'translate-x-full'
				} lg:none`}
			>
				<ul className='flex flex-col absolute top-0 right-0 h-full w-8/12 px-5 py-8 bg-offWhite'>
					<svg
						width='32'
						height='31'
						xmlns='http://www.w3.org/2000/svg'
						className='self-end cursor-pointer mb-32'
					...
					{navLinks.map((link) => {
						return (
							<li className='py-4' key={uuid()}>
								<a
									className='text-darkGrayishBlue text-2xl hover:text-primaryOrange transition ease-in-out'
									href={link.linkTarget}
									onClick={() => setIsOpen(false)}
								>
									{link.linkName}
